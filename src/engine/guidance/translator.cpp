...
#include "engine/guidance/translator.hpp"
...
{
    const constexpr char *base[detail::num_direction_modifiers] = {
        "After a powerslide, the road name changes to ",
        "The road takes a sharp turn to the right and becomes ",
        "The road turns right and becomes ",
        "The road takes a slight turn to the right and becomes ",
        "The road goes straight into ",
        "The road takes a slight turn to the left and becomes ",
        "The road turns left and becomes ",
        "The road takes a sharp turn to the left and becomes "};

    return std::string(base[static_cast<int>(modifier)]) +
           ((name == "") ? "an unnamed road" : name);
}
...
inline std::string translateRamp(const DirectionModifier modifier, const std::string &name)
{
    const constexpr char *base[detail::num_direction_modifiers] = {
        ...
        "Turn right and take the ramp",     "Take the ramp",
        "Go straight onto the ramp",        "Take the ramp on the left",
        "Turn left and take the ramp",      "Make a sharp left and take the ramp"};
...
inline std::string translateFork(const DirectionModifier modifier, const std::string &name)
{
    ...
    return std::string("At the fork, keep ") +
           ((modifier == DirectionModifier::SlightLeft) ? "left" : "right") +
...
std::string translate(const TurnInstruction instruction, const std::string &name)
{
    ...
    default:
        return "Instruction (type: " + std::to_string(static_cast<int>(instruction.type)) +
               ", direction_modifier: " +
               std::to_string(static_cast<int>(instruction.direction_modifier)) + ") not handled";
    }
}
...
std::string translateRoundabout(const LocationType location, const std::string &name, int exit)
{
    ...
    {
        return "Enter the roundabout" + std::to_string(exit) +
               ((name != "") ? (" (" + name + ")") : "");
    }
    return std::string("Take exit Nr ") + std::to_string(exit) +
           ((name != "") ? (" onto " + name) : "");
}
